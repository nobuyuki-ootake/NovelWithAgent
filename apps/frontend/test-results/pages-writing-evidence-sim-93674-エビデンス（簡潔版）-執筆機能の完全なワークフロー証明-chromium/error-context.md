...
- Name: 執筆機能エビデンス（簡潔版） >> 執筆機能の完全なワークフロー証明
- Location: C:\Users\irure\git\NovelWithAgent\apps\frontend\e2e\pages\writing-evidence-simplified.spec.ts:18:3
...
Call log:
  - waiting for locator('div').filter({ hasText: /^テストプロジェクト作成日:.*更新日:.*$/ }).first()
    ...
    at C:\Users\irure\git\NovelWithAgent\apps\frontend\e2e\pages\writing-evidence-simplified.spec.ts:28:19
```
