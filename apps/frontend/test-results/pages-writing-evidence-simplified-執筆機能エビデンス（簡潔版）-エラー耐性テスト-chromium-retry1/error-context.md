...
- Name: 執筆機能エビデンス（簡潔版） >> エラー耐性テスト
- Location: C:\Users\irure\git\NovelWithAgent\apps\frontend\e2e\pages\writing-evidence-simplified.spec.ts:189:3
...
Call log:
  - waiting for locator('div').filter({ hasText: /^テストプロジェクト作成日:.*更新日:.*$/ }).first()
    ...
    at C:\Users\irure\git\NovelWithAgent\apps\frontend\e2e\pages\writing-evidence-simplified.spec.ts:192:19
```
